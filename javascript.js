...
function checkAnswer() {
    ...
    if (selectedAnswer === questions[currentQuestion].aperture) {
        feedbackElement.textContent = `True aperture: ${correctAnswer}. Current score: ${Math.round(absScore*100)/100}.`;
        feedbackElement.classList.add('correct');
        feedbackElement.classList.remove('incorrect');
    } else {
        distance = (fstops.indexOf(selectedAnswer) - fstops.indexOf(questions[currentQuestion].aperture))/3;
        absScore += Math.abs(distance);
        score += distance;
        feedbackElement.textContent = `True aperture: ${correctAnswer} (Error: ${(distance<0?"":"+") + Math.round(distance*100)/100} stops). Current score: ${Math.round(absScore*100)/100} stops.`;
...
function showScore() {
    ...
    scoreElement.textContent = `Final score: ${Math.round(absScore*100)/100} stops. Average absolute error:  ${Math.round(absScore/questions.length*100)/100} stops.  Average error:  ${Math.round(score/questions.length*100)/100} stops. `;
